...
// State that must survive between injections is stashed on `window.__clippyFullpage` (the
...
export function gotoTile(x, y, hideSticky) {
  const state = window.__clippyFullpage;
  if (state) {
...
export function restorePageAfterCapture() {
  ...
  delete window.__clippyFullpage;
}
