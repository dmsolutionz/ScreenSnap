// Injected on-page control for Current-Tab recording (self-contained). Shows a 3-2-1 countdown, then
// a compact pill (timer · pause · stop) so the recording can always be seen and stopped without the
// popup — the popup closes on blur, which previously left tab recordings stranded. The pill lives on
// the recorded tab, so it appears in the capture; it sits bottom-centre and stays minimal.
// Message strings mirror src/lib/messages.js (content scripts can't import the module).
(() => {
  ...
  function Control() {
    ...
    root.innerHTML = `
      <style>
        :host { all: initial; }
        .center { position: fixed; inset: 0; z-index: 2147483647; display: flex; align-items: center; justify-content: center;
        ...
        .pill { position: fixed; left: 50%; bottom: 22px; transform: translateX(-50%); z-index: 2147483647;
          background: rgba(5,5,5,0.92); backdrop-filter: blur(14px); border: 1px solid rgba(255,255,255,0.12);
          border-radius: 30px; padding: 9px 12px 9px 16px; display: none; gap: 12px; align-items: center;
          box-shadow: 0 8px 24px rgba(0,0,0,0.55); user-select: none; }
        .rec { width: 9px; height: 9px; border-radius: 50%; background: #ef4444; box-shadow: 0 0 8px rgba(239,68,68,0.6); animation: rp 1.2s infinite; }
    ...
    // Nothing to wait for (no camera) — count straight down, then begin the capture.
    let n = 3;
    ...
    const cd = setInterval(() => {
      ...
      if (n <= 0) {
        ...
        send("rec-go"); // tells the service worker to start capturing now
      } else tick();
    }, 1000);
    ...
    this.onState = (s) => {
      ...
      if (phase === "recording") { started = true; pill.style.display = "flex"; root.getElementById("center")?.remove(); }
    ...
    this.destroy = () => { clearInterval(interval); clearInterval(cd); host.remove(); window.__screensnapCtl = null; };
  }
})();
